...
import requests  #to work with external URLs
...
def get_hotels() -> List[dict]:
    ...
    """
    data = _fetch_json(HOTEL_API_BASE) #retrieve all data of hotels
...
def get_flights() -> List[dict]:
    #we add a docstring to each tool to specify the Do's so it will identify the specific tool according to the docstring
    """
...
def search_hotel(
    ...
    data = _fetch_json(f"{HOTEL_API_BASE}/search", params=params)#we passs parameters because we are searching the hotel API Base using these parameters
...
def book_hotel(
    ...
    response = requests.post(f"{HOTEL_API_BASE}/book", json=payload) ##payload as a json input///first time we are using a post request
    return response.json()
