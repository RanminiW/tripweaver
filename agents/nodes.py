...
from .mcp_tools import get_tool, parse_mcp_result
from .llm import llm
from .prompts import get_system_prompt_with_history, get_system_prompt_for_unknown_node#with history
...
async def hotel_node(state: GraphState) -> dict:
    ...
    if state.get("sub_action") == "book":
        ...
        missing = [
            field for field, value in [
                ...
                ("guest_email", guest_email), ("check_in", check_in_date),
                ("check_out", check_out_date), ("room_type", room_type),
            ] if not value
        ]
        ...
        try:
            result = await get_tool("book_hotel").ainvoke({
                "hotel_id": hotel_id,
            ...
            print("RAW HOTEL BOOKING RESULT:", result)
        except Exception:
            return {
                "hotel_results": [],
                "flight_results": [],
                "response_text": (
                    "Sorry, the hotel booking service is temporarily unavailable. "
                    "Please try again shortly."
                ),
            }

        # after parsing, a single-object tool result comes back as a one-item list
        booking_data = result[0] if isinstance(result, list) and result else result

        if isinstance(booking_data, dict) and booking_data.get("success"):
            booking = booking_data.get("booking", {})
            confirmation = (f"Hotel booked! Confirmation reference: {booking.get('bookingReference', 'N/A')}, "
                            f"{booking.get('numberOfNights', 'N/A')} night(s), "
                            f"total {booking.get('totalPrice', 'N/A')} - status: {booking.get('status', 'confirmed')}."
                            )
            return {"hotel_results": [], "flight_results": [], "response_text": confirmation}
        return {"hotel_results": [], "flight_results": [], "response_text": "Hotel booking completed."}
    ...
    try:
        if city:
            ...
            result = await get_tool("search_hotel").ainvoke(params)
        else:
        ...
        return {
            ...
            "flight_results": [],
            "response_text": (
                "Sorry, the hotel search service is temporarily unavailable. "
                "Please try again shortly, or ask about flights in the meantime."
            ),
        }
...
async def flight_node(state: GraphState) -> dict:
    ...
    if state.get("sub_action") == "book":
        ...
        try:
            result = await get_tool("book_flight").ainvoke({
                "flight_id": flight_id,
                "passenger_name": passenger_name,
                "passenger_email": passenger_email,
            })
            result = parse_mcp_result(result)
            print("RAW BOOKING RESULT:", result)
        except Exception:
            return {
                "hotel_results": [],
                "flight_results": [],
                "response_text": (
                    "Sorry, the flight booking service is temporarily unavailable. "
                    "Please try again shortly."
                ),
            }
        ...
        if isinstance(booking_data, dict) and booking_data.get("success"):
            booking = booking_data.get("booking", {})
            confirmation = (
                f"Flight booked! Confirmation reference: {booking.get('bookingReference', 'N/A')}, "
                f"seat {booking.get('seatNumber', 'N/A')}, status: {booking.get('status', 'confirmed')}."
                )
            return {"hotel_results": [], "flight_results": [], "response_text": confirmation}
        return {"hotel_results": [], "flight_results": [], "response_text": "Flight booking completed."}
    ...
    try:
        if origin and destination:
            ...
            result = await get_tool("search_flights").ainvoke(params)
        else:
        ...
        return {
            ...
            "flight_results": [],
            "response_text": (
                "Sorry, the flight search service is temporarily unavailable. "
                "Please try again shortly, or ask about hotels in the meantime."
            ),
        }
